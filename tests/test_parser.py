...
def test_nested_datetime_conversion(nested_data):
    ...
    with patch("json.dumps") as mock_dumps, patch("pygments.highlight"), patch("builtins.print"):
...
def test_valid_json_list(tmp_path):
    ...
    result = read_file_from_json(str(file_path))
    assert result == [1, 2, 3]
...
def test_file_not_found():
    ...
    result = read_file_from_json("non_existent_file.json")
    assert result == []
