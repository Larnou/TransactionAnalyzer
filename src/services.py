...
def get_transaction_history(transaction_data: DataFrame, year: str | int, month: str) -> list[dict[str, Any]]:
    ...
    service_cashback_logger.info(f"Список транзакций transaction_history состоит из: {len(transaction_history)} записей.")
    service_cashback_logger.info(f"Вывод списка транзакций transaction_history.")
...
def service_cashback(transaction_data, year_for_analyze, month_for_analyze):
    ...
    service_cashback_logger.info(f"Выполнена сортировка списка категорий за указанный период.")
